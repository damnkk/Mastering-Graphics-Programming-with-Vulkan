...
// clang-format off
static const char *kDefault3DModel =
    "E:/repository/Mastering-Graphics-Programming-with-Vulkan/deps/src/glTF-Sample-Models/2.0/Sponza/glTF/Sponza.gltf";
// clang-format on
...
static_assert(INVALID_INT_VALUE == i32_max,
              "Mismatch between invalid int and i32 max");
...
struct AnimationChannel {
  ...
  enum TargetType { Translation, Rotation, Scale, Weights, Count };
...
struct AnimationSampler {
  i32 input_keyframe_buffer_index;  //"The index of an accessor containing
                                    // keyframe timestamps."
  i32 output_keyframe_buffer_index; // "The index of an accessor, containing
                                    // keyframe output values."

  enum Interpolation { Linear, Step, CubicSpline, Count };
  ...
  // **SHOULD** be used to interpolate quaternions. The float of output elements
  // **MUST** equal the float of input elements. STEP The animated values remain
  // constant to the output of the first keyframe, until the next keyframe. The
  // float of output elements **MUST** equal the float of input elements.
  // CUBICSPLINE The animation's interpolation is computed using a cubic spline
  // with specified tangents. The float of output elements **MUST** equal three
  // times the float of input elements. For each input element, the output
...
struct Material {
  ...
  // exact appearance of the edges **MAY** be subject to implementation-specific
  // techniques such as "`Alpha-to-Coverage`". BLEND The alpha value is used to
  // composite the source and destination areas. The rendered output is combined
  // with the background using the normal painting operation (i.e. the Porter
...
struct Sampler {
  ...
  enum Wrap { CLAMP_TO_EDGE = 33071, MIRRORED_REPEAT = 33648, REPEAT = 10497 };
...
i32 gltf_get_attribute_accessor_index(
    glTF::MeshPrimitive::Attribute *attributes, u32 attribute_count,
